...
int busca_binaria(int arr[], int tamanho, int elemento, int *contador) {
    int esquerda = 0, direita = tamanho - 1;

    while (esquerda <= direita) {
        int meio = esquerda + (direita - esquerda) / 2;
        (*contador)++;  // Incrementa o contador a cada cálculo de 'meio'
...
int main(int argc, char *argv[]) {
    ...
    int resultado = busca_binaria(arr, tamanho, elemento, &contador);
    if (resultado != -1)
        printf("Elemento %d encontrado no índice %d\n", elemento, resultado);
    else
        printf("Elemento %d não encontrado\n", elemento);

    printf("A variável 'meio' foi calculada %d vezes\n", contador);
